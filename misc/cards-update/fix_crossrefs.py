...
with open("cardbase_crypt.json", "r+") as crypt_file, open(
    ...
    "../../frontend/src/assets/data/disciplinesList.json", "r"
) as disciplines_list, open(
    ...
    def generate_crossref(card):
        for other_card in cards.values():
            ...
            if name not in blacklist and name not in card["name"] and name in card["text"]:
                ...
                if f"/{name}/" in card["text"] or is_long_name_exception:
                    ...
                    card["Card Text"] = card["text"].replace(name, f"/{other_card['name']}/")
