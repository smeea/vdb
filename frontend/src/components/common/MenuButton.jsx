...
const MenuButton = ({ title, icon, variant = "primary", className, text }) => {
  ...
  return (
    <Menu.Button
      className={twMerge(
        "flex min-h-[41px] cursor-pointer items-center justify-center gap-2 rounded-sm px-3 py-1.5 text-fgThird outline-bgCheckboxSelected focus:outline dark:text-fgThirdDark dark:outline-bgCheckboxSelectedDark",
        mainStyle[variant],
        className,
      )}
      title={title}
