...
const TwdSearchForm = ({ error, setError }) => {
  const { cryptCardBase, libraryCardBase, showFloatingButtons, inventoryMode, isMobile, limitedMode, limitedPreset } = useApp();
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {limitedMode && limitedPreset === V5 && (
        <>
          <TwdSearchFormMatchInventory
            value={twdFormState[MATCH_V5][CRYPT]}
            target={CRYPT}
            onChange={handleChangeWithOpt}
            name={MATCH_V5}
            text="V5 by"
          />
          <TwdSearchFormMatchInventory
            value={twdFormState[MATCH_V5][LIBRARY]}
            target={LIBRARY}
            onChange={handleChangeWithOpt}
            name={MATCH_V5}
            text="V5 by"
