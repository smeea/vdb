...
import { Button, ButtonIconed, FlexGapped, Header, Modal } from "@/components";
import { ALL, CRYPT, ID, INVENTORY_KEY, LIBRARY, OK, SURPLUS, SURPLUS_KEY } from "@/constants";
...
const InventoryShareModal = ({ setShow }) => {
  ...
  Object.values(useInventoryCrypt(inventoryCrypt, OK, false, ALL, false)[SURPLUS]).forEach((v) => {
    surplus[v.c[ID]] = v.q;
  });

  Object.values(
    useInventoryLibrary(inventoryLibrary, OK, false, ALL, ALL, ALL, false)[SURPLUS],
  ).forEach((v) => {
    surplus[v.c[ID]] = v.q;
  });
  ...
  const handleCreate = (target) => {
    ...
    const createUrl =
      target === INVENTORY_KEY
        ? inventoryServices.shareFullInventory
        : inventoryServices.shareSurplusInventory;
  ...
  const handleUpdateSurplus = () => {
    inventoryServices
      .shareSurplusInventory(surplusKey, surplus)
      .then(() => {
        navigator.clipboard.writeText(
          `${import.meta.env.VITE_BASE_URL}/inventory?key=${surplusKey}`,
        );
      })
      .then(() => {
  ...
  return (
    <Modal handleClose={handleClose} centered={isMobile} size="xs" title="Share Inventory">
      <FlexGapped className="flex-col">
        <div className="flex flex-col gap-0.5 rounded-sm border border-borderPrimary dark:border-borderSecondaryDark">
          ...
          <div className="flex flex-col gap-2 p-2">
            ...
            <div className="flex flex-col gap-1">
              <p>Shows all cards from you inventory.</p>
              <p>Will follow changes in your inventory.</p>
            </div>
            <ButtonIconed
              className="w-full"
          ...
          <div className="flex flex-col gap-2 p-2">
            ...
            <div className="flex flex-col gap-1">
              <p>
                Shows surplus cards{" "}
                <b>
                  <i>as they were at the moment of URL creation</i>
                </b>
                , or when you click UPDATE button.
              </p>
              <p>Update button will update shared surplus to current state without changing URL.</p>
              <p>Will not follow changes in your inventory.</p>
            </div>
        ...
        <div className="flex flex-col gap-1">
          <div>People with URL can only view (not edit!) your inventory.</div>
