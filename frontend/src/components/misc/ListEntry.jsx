...
const ListEntry = ({ icon, title, children, forceOneLine, basis = 4 }) => {
  const basises = {
  ...
  return (
    <div
      className={`flex gap-2 ${forceOneLine ? '' : 'max-sm:flex-col'} sm:items-center sm:justify-between sm:gap-4`}
    >
      <div
      ...
      <div className={`flex items-center ${basises[basis][1]}`}>{children}</div>
    </div>
  );
};
